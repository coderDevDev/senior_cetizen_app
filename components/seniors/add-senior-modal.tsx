...
export function AddSeniorModal({
  ...
  const onSubmit = async (data: AddSeniorFormData) => {
    ...
    try {
      ...
      console.log('Adding senior citizen:', formData);

      // TODO: Implement API call to add senior citizen
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-2xl font-bold text-[#333333]">
            <User className="w-6 h-6 text-[#00af8f]" />
            Add New Senior Citizen
          </DialogTitle>
          <DialogDescription className="text-[#666666]">
            Register a new senior citizen in the system. All fields marked with
            * are required.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              disabled={isLoading}
              className="bg-[#00af8f] hover:bg-[#00af90] text-white">
              {isLoading ? 'Adding...' : 'Add Senior Citizen'}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
